...
vn_t NamesPass::getVarName(const Value *V) {
  if (!V) {
    errs() << "WARNING : VarName requested for null value.\n";
    return nullptr;
  }
...
void NamesPass::updateMemory(MemoryName index, vn_t update) {
  ...
  } else if (update->type == VarType::FUNCTION &&
        backing_name && update->name() != backing_name->name()) {
  ...
  if (callee->type == VarType::FUNCTION) {
    ...
    for (vn_t fn : multi_callee->names()) {
      if (fn->type != VarType::FUNCTION) {
        errs() << "IMPRECISION WARNING: Call through non-function Value. VarName type is "
               << fn->getTypeAsStr() << "\n";
        continue;
      }
...
// To pass test191
// %9 = trunc i64 %8 to i32, used by LLVM for return value of PTR_ERR
void NamesPass::visitTruncInst(TruncInst &I) {
  names[&I] = getVarName(I.getOperand(0));
}

// To pass test250
// %10 = bitcast i8* %9 to i32*, used by LLVM for return value of ERR_PTR
void NamesPass::visitBitCastInst(BitCastInst &I) {
  names[&I] = getVarName(I.getOperand(0));
}

// To pass test250
void NamesPass::visitSExtInst(SExtInst &I) {
  names[&I] = getVarName(I.getOperand(0));
}
...
void NamesPass::visitGetElementPtrInst(GetElementPtrInst &I) {
  ...
  if (struct_name && struct_name->type != VarType::MULTI && struct_name->type != VarType::EC) {
  ...
  } else if (!struct_name || (struct_name && struct_name->type == VarType::EC)) {
