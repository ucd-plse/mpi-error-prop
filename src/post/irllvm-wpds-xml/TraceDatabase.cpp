...
sqlite3_int64 TraceDatabase::addHandlerTrace(const Trace &trace) {
  ostringstream query;
  query << "INSERT INTO Handler (stack, predicate_loc) VALUES ('"
        << trace.stack_id << "','" << trace.location << "');";
  char *errMsg = nullptr;
...
void TraceDatabase::addNested(const sqlite3_int64 parent_id, const sqlite3_int64 child_id) {
  ostringstream query;
  query << "INSERT INTO Nesting (parent_handler, child_handler) VALUES ("
	<< parent_id << "," << child_id << ");";
...
void TraceDatabase::initialize() {
  ...
  string query = "CREATE TABLE Handler(id INTEGER PRIMARY KEY, stack TEXT, predicate_loc TEXT);";
