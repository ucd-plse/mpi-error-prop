...
bool ReachableHandlersVisitor::follow_edge(const flow_edge_t edge, const _FlowGraph &G) const {
  if (!G[edge].call) {
    return true;
  }
  return false;
...
void ReachableHandlersVisitor::discover_vertex(flow_vertex_t vtx, _FlowGraph &G) {
  G[vtx].visited = true;

  string handler = G[vtx].stack;
  auto stop = handlers_stop.find(handler);
  if (stop != handlers_stop.end()) {
    auto it = handler2instances.find(handler);
    if (it != handler2instances.end()) {
      std::set<Location> connected_instances = it->second;
      std::set<Location> common_instances;
      std::set_intersection(begin(instances), end(instances),
                            begin(connected_instances), end(connected_instances),
                            std::inserter(common_instances, common_instances.begin()));

      if (!common_instances.empty() && handler != handler_start) {
        reachable.push_back(handler);
      }
    }
  }
}
