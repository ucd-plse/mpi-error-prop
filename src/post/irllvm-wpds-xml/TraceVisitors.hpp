...
class ReachableHandlersVisitor : public ep::DFSVisitorInterface<_FlowGraph> {
public:
  ReachableHandlersVisitor(std::map<string, string> &handlers_stop,
                           std::map<string, std::set<Location>> &handler2instances,
                           string handler_start,
                           std::set<Location> &instances) :
      handlers_stop(handlers_stop),
      handler2instances(handler2instances),
  ...
  vector<string> getReachable() { return reachable; }
