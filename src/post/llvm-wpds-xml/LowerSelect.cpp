...
namespace {
  ...
  public:
    ...
    explicit LowerSelect(bool onlyfp = false) : FunctionPass(ID), // FunctionPass((intptr_t)&ID) 
      OnlyFP(onlyfp) {}
  ...
  for (Function::iterator BB = F.begin(), E = F.end(); BB != E; ++BB)
    for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E; ++I) {
      if (SelectInst *SI = dyn_cast<SelectInst>(I))
        if (!OnlyFP || SI->getType()->isFloatingPointTy()) { // isFloatingPoint
          ...
          BasicBlock *NewTrue = BasicBlock::Create(BB->getContext(), BB->getName()+".selecttrue",
	  ...
	  PHINode *PN = PHINode::Create(SI->getType(), "", NewCont->begin());
          PN->takeName(SI);
