/**
 * @author Nicholas Kidd, Modified by Cindy Rubio Gonzalez
...
namespace wali
{
  namespace wpds
  {
    namespace ewpds
    {
	  ...
	  string msgcode = "overwrite";
          if (toStk2Key == WALI_EPSILON) 
          {
	    if (toStk1Key == WALI_EPSILON) {
	      msgcode = "outOfScope";
	    }
            ...
            get_ewpds().add_rule(
                ...
                fUserHandler.getWeight(msgcode, fromStkKey));
          }
          else {
	    // Push Rule
            //std::cerr << " . Weight S = " << weightString << std::endl;
            ...
            if (fUserHandler.hasMergeFn())
            {
              get_ewpds().add_rule(
                  ...
                  fUserHandler.getMergeFn(se, fromStkKey));
            }
            else
            {
              get_ewpds().add_rule(
